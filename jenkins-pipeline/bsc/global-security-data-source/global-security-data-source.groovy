      ...
      yaml """
        ...
        spec:
          ...
          - name: jnlp
            ...
            tty: true
          - name: package-tools
            image: "${office_registry}/libs/rust-uv-python:1.80.1-0.2.29-3.12.4"
            imagePullPolicy: Always
            env:
              - name: "file.encoding"
                value: "UTF-8"
          ...
          script {
            ...
            withCredentials([usernameColonPassword(credentialsId: '3d040389-9dfe-4c0d-9dab-9f6487f10409', variable: 'USERPASS')]) {
              sh """
                ...
                uv python list
                uv venv --python 3.12.4 --python-preference only-managed
                #. .venv/bin/activate
        ...
        container('podman') {
          script{
            for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
              echo "创建Dockerfile"
              dockerFile = """
                FROM ${office_registry}/libs/python:bf-3.12.4-bookworm
                LABEL maintainer="colin" version="1.0" datetime="2024-08-16"
                COPY .venv/lib/python3.12/site-packages /usr/local/lib/python3.12/site-packages
                COPY dist/*.whl /opt/betack/
                WORKDIR /opt/betack/
                RUN pip install *.whl
              """.stripIndent()
