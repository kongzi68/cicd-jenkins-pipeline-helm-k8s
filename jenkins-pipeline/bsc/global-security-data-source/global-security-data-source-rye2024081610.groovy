...
def git_address = "ssh://git@code.betack.com:4022/betack/global-security-data-source.git"
...
pipeline {
      ...
      yaml """
        ...
        spec:
          ...
          - name: jnlp
            ...
            tty: true
          - name: package-tools
            image: "${office_registry}/libs/rye-python:3.11.9-rye"
            imagePullPolicy: Always
            env:
              - name: "file.encoding"
                value: "UTF-8"
  ...
  parameters {
    ...
    extendedChoice description: '请选择本次发版需要部署的服务',
                   ...
                   type: 'PT_CHECKBOX',
                   value: 'global-security-data-source',
                   visibleItemCount: 8
    extendedChoice description: '发版到选中的运行环境',
                   ...
                   saveJSONParameterToFile: false,
                   type: 'PT_CHECKBOX',
                   value: 'saasdata-dev-1,saasdata-dev-2',
                   visibleItemCount: 8
  }
          ...
          script {
            ...
            withCredentials([usernameColonPassword(credentialsId: '3d040389-9dfe-4c0d-9dab-9f6487f10409', variable: 'USERPASS')]) {
              sh """
                ...
                export RYE_HOME=${WORKSPACE}/.rye
                rye --version
                #rye toolchain remove 3.11.9
                #rye toolchain register /usr/local/bin/python3
              ...
              """.stripIndent()
              writeFile file: "${WORKSPACE}/.rye/config.toml", text: "${ryeConfig}", encoding: 'UTF-8'
              sh """
                export RYE_HOME=${WORKSPACE}/.rye
                rye config --set proxy.http=http://IAmIPaddress:7890
                rye config --set proxy.https=http://IAmIPaddress:7890
          ...
          script{
            for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
              ...
              if (params.IS_ENABLED_UPDATE_DEPENDENCIES) {
                ...
                dockerFile = """
                  FROM ${office_registry}/rabbeyond/global-security-data-source:latest
                  LABEL maintainer="colin" version="1.0" datetime="2024-07-17"
                  COPY src /opt/betack/src
                  WORKDIR /opt/betack/
                  #CMD ["python", "src/__main__.py"]
        ...
        script {
          ...
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            ...
            for (namespaces in params.DEPLOY_TO_ENV.tokenize(',')) {
              helm_values_file="temp_jenkins_workspace/bsc/global-security-data-source/deployToK8s/${deploy_svc_name}-values.yaml"
              ...
              // 存储卷
              svcYaml['storage']['storageEnable'] = false
              svcYaml['storage']['isCreateDataPVC'] = false
              svcYaml['storage']['isMountDataPV'] = false
