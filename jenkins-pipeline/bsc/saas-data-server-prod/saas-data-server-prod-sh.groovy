...
pipeline {
  ...
  parameters {
    ...
    extendedChoice description: '请选择本次发版需要部署的服务',
                   ...
                   type: 'PT_CHECKBOX',
                   value: 'data-etl-server,data-subscriber-client,etl-listener,data-migration-job,data-meta-import-job',
                   visibleItemCount: 8
    ...
    extendedChoice description: '发版到选中的运行环境',
                   ...
                   saveJSONParameterToFile: false,
                   type: 'PT_CHECKBOX',
                   value: 'saasdata-prod-1,saasdata-single-prod-2',
                   visibleItemCount: 10
        ...
        script {
          ...
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            ...
            for (namespaces in params.DEPLOY_TO_ENV.tokenize(',')) {
              switch(namespaces) {
                case ['saasdata-prod-1']:
                  k8s_pv_size = '1536Gi'
                break
                case ['saasdata-single-prod-2']:
                  k8s_pv_size = '1024Gi'
                break
              }
              // 上海idc prod k8s集群
                ...
                if (isFirst) {
                  ...
                  switch(deploy_svc_name) {
                    case ['data-etl-server']:
                      ...
                      svcYaml['storage']['capacity'] = k8s_pv_size
                      svcYaml['storage']['dataPVCMountPath'] = '/opt/saas/data'
