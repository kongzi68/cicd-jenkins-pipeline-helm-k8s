...
def git_address = "ssh://git@code.betack.com:4022/sfinx/sfinx-java-formula2.git"
      ...
      yaml """
        ...
        spec:
          ...
          - name: tools
            ...
            tty: true
          - name: tools-build
            image: "${office_registry}/libs/maven:3.9.9-amazoncorretto-17-debian"
    ...
    stage('环境准备') {
      steps {
        script {
          ...
          // 手动指定jdk版本，为过安全检查，用最新的base镜像看看效果
          // IAmIPaddress:8765/libs/azul/zulu-openjdk:11.0.23-11.72
          // JDKVERSION = "${office_registry}/libs/azul/zulu-openjdk:11.0.23-11.72"
          println(JDKVERSION)
          // 执行命令 gradle clean
          // gradleClean()
        }
          ...
          script {
            withCredentials([usernameColonPassword(credentialsId: '3d040389-9dfe-4c0d-9dab-9f6487f10409', variable: 'USERPASS')]) {
              sh """
                ...
                apt-get clean all; apt-get update; apt-get install -y git
                #[ -d '/iamusername/.m2' ] && ls -lh /iamusername/.m2/ || mkdir -p /iamusername/.m2
                #cp -a ${WORKSPACE}/.m2/settings.xml /iamusername/.m2/
                echo 'http://${USERPASS}@IAmIPaddress:14480' > ${WORKSPACE}/git-credentials
                git config --global credential.helper 'store --file ${WORKSPACE}/git-credentials'
              """
              for (deploySVCName in params.DEPLOY_SVC_NAME.tokenize(',')) {
                echo "当前正在构建服务：${deploySVCName}"
                tempDeploySVCName = deploySVCName.replaceAll("-","_")
                JAR_PKG_NAME = "target/${tempDeploySVCName}*-SNAPSHOT.jar"
                sh """
                  git submodule update --init --recursive
                  git submodule update --remote
                  mvn -s ${WORKSPACE}/.m2/settings.xml -Dmaven.repo.local=${WORKSPACE}/.m2/repository package -q
                """
        ...
        container('podman') {
          script{
            for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
              tempDeploySVCName = deploy_svc_name.replaceAll("-","_")
              pkgInfo = libTools.getJarPackageName("target/${tempDeploySVCName}*-SNAPSHOT.jar")
              TEMP_JAR_NAME = pkgInfo[0]
              JAR_PKG_NAME = pkgInfo[1]
              dockerFile = """
                  FROM ${JDKVERSION}
                  LABEL maintainer="colin" version="1.0" datetime="2024-07-17"
                  RUN ln -sf /usr/share/zoneinfo/Asia/Shanghai /etc/localtime && \
                      echo "Asia/Shanghai" > /etc/timezone
                  COPY ${TEMP_JAR_NAME} /opt/betack/${deploy_svc_name}.jar
                  WORKDIR /opt/betack
              """.stripIndent()
    ...
    stage('部署服务') {
      ...
      steps {
        script {
          ...
          // 循环处理需要部署的服务
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            helm_values_file="temp_jenkins_workspace/${project}/sfinx-java-formula2/deployToK8s/${deploy_svc_name}-values.yaml"
            // 循环处理需要部署的命名空间
            for (namespaces in params.DEPLOY_TO_ENV.tokenize(',')) {
              ...
              svcYaml['commandOps'] = [reserveJavaCommand(k8sRSType: 'deploy', namespaces: namespaces, deploySVCName: deploy_svc_name)]
              ...
              if (isFirst) {
                ...
                svcYaml['storage']['dataPVCNameInfix'] = 'sfinx-java-formula2'
                svcYaml['storage']['dataPVCMountPath'] = '/opt/betack/data'
