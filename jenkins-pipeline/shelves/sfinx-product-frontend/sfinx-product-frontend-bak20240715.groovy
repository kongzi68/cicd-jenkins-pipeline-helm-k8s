...
pipeline {
  ...
  parameters {
    ...
    extendedChoice description: '发版到选中的运行环境',
                   ...
                   type: 'PT_CHECKBOX',
                   value: 'product-rack-dev-1,product-rack-dev-2,product-rack-staging-1',
                   visibleItemCount: 8
  ...
  stages {
    ...
    stage('代码编译打包') {
      steps {
        echo '正在构建...'
        script {
          frontendCodeCompile(buildScripts: "yarn install; yarn add sharp; npm run build;")
        }
      }
    }
      ...
      steps {
        container('podman') {
          script {
            ...
            // error '调试退出'
            // 镜像需瘦身
            // 参考资料：https://github.com/vercel/next.js/blob/canary/examples/with-docker/Dockerfile
            // https://nextjs.org/docs/pages/building-your-application/deploying#docker-image
            dockerFile = """
              FROM ${office_registry}/libs/node:18.17.0-alpine
              LABEL maintainer="colin" version="1.0" datetime="2024-06-05"
              WORKDIR /app
              ...
              #COPY static/images ./.next/static/images
              RUN chown -R nextjs:nodejs /app
              USER nextjs
              EXPOSE 3000
              ENV PORT 3000
              CMD HOSTNAME="IAmIPaddress" node server.js
            """.stripIndent()
