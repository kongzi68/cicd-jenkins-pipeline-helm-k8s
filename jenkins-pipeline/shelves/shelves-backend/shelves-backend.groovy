...
def git_address = "ssh://git@code.betack.com:4022/betack/shelves-backend.git"
...
pipeline {
  ...
  options {
    ...
    buildDiscarder logRotator(artifactDaysToKeepStr: '', artifactNumToKeepStr: '', numToKeepStr: '60')  // 设置保留30次的构建记录
  }
  ...
  parameters {
    ...
    extendedChoice defaultValue: '11',
                   ...
                   value: '8,11,17',
                   visibleItemCount: 4
    extendedChoice defaultValue: 'gradle-8.5',
    ...
    extendedChoice description: '请选择本次发版需要部署的服务',
                   ...
                   saveJSONParameterToFile: false,
                   type: 'PT_CHECKBOX',
                   value: 'shelves-app,shelves-data-sync',
                   visibleItemCount: 6
    extendedChoice description: '发版到选中的运行环境',
                   ...
                   saveJSONParameterToFile: false,
                   type: 'PT_CHECKBOX',
                   value: 'product-rack-dev-1,product-rack-dev-2,product-rack-staging-1,product-rack-staging-2',
                   visibleItemCount: 10
    booleanParam defaultValue: false,
      ...
      steps {
        ...
        script {
          for (deploySVCName in params.DEPLOY_SVC_NAME.tokenize(',')) {
            ...
            JAR_PKG_NAME = "${deploySVCName}/build/libs/${deploySVCName}-1.0-SNAPSHOT.jar"
    ...
    stage('部署服务') {
      ...
      steps {
        script {
          ...
          // 循环处理需要部署的服务
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            helm_values_file="temp_jenkins_workspace/shelves/shelves-backend/deployToK8s/${deploy_svc_name}-values.yaml"
            // 循环处理需要部署的命名空间
