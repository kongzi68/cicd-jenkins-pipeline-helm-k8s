...
pipeline {
  ...
  parameters {
    ...
    extendedChoice description: '请选择本次发版需要部署的服务',
                   ...
                   type: 'PT_CHECKBOX',
                   value: 'fund-report,fund-evaluation-mobile,counsel-nginx,rab-webapp,portfolio-risk-research-management',
                   visibleItemCount: 6
    extendedChoice description: '发版到选中的运行环境, 其中：\nofficial-website-prod-1 是官网的API; \nproduct-rack-prod-1 是策略产品货架API',
                   ...
                   saveJSONParameterToFile: false,
                   type: 'PT_CHECKBOX',
                   value: 'rab-api3-prod-1,rab-api4-prod-1,rab-api5-prod-1,rab-api5-uat-1,official-website-prod-1,product-rack-prod-1',
                   visibleItemCount: 8
  }
          ...
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            ...
            for (namespaces in params.DEPLOY_TO_ENV.tokenize(',')) {
              ...
              if (isFirst) {
                ...
                switch(namespaces) {
                  case ['rab-api5-prod-1']:
                    API_ADDRS = "http://IAmIPaddress:7080"
                    SOCKET_ADDRS = "http://IAmIPaddress:9999/socket.io"
                  break
                  case ['rab-api4-prod-1']:
                    API_ADDRS = "http://rab-svc-api-app-out-${configEnvSuffix}:80"
                    SOCKET_ADDRS = "http://rab-svc-api-app-out-${configEnvSuffix}:9999/socket.io"
                  break
                  default:
