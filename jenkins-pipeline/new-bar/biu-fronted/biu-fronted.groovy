...
def git_address = 'ssh://git@code.betack.com:4022/betack/biu-fronted.git'
...
pipeline {
  ...
  options {
    ...
    buildDiscarder logRotator(artifactDaysToKeepStr: '', artifactNumToKeepStr: '', numToKeepStr: '100')  // 设置保留30次构建记录
  }
  ...
  parameters {
    ...
    extendedChoice description: '发版到选中的运行环境',
                   ...
                   type: 'PT_CHECKBOX',
                   value: 'new-rab-dev-3,new-rab-dev-4',
                   visibleItemCount: 10
  }
    ...
    stage('环境准备') {
      steps {
        container('frontend') {
          ...
          script {
            ENV_TYPE = 'pnpm'  // 可选值 npm、yarn、pnpm
            selectFrontendBuildEnv(envType:ENV_TYPE, nodeVersion:'18.17.0', pnpmVersion:'8.11.0')
          }
        }
      }
    }
    ...
    stage('代码编译打包') {
      steps {
        container('frontend') {
          echo '正在构建...'
          script {
            frontendCodeCompile(buildScripts:'pnpm install; CI=false pnpm run build')
          }
        }
      }
    }
      ...
      steps {
        script {
          ...
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            helm_values_file="temp_jenkins_workspace/new-${project}/biu-fronted/deployToK8s/${deploy_svc_name}-values.yaml"
            // 循环处理需要部署的命名空间
