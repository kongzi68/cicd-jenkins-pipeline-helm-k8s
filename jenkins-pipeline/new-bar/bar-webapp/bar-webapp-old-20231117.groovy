...
def git_address = 'ssh://git@code.betack.com:4022/betack/rab-webapp.git'
// 认证
...
pipeline {
  ...
  parameters {
    ...
    extendedChoice description: '发版到选中的运行环境',
                   ...
                   type: 'PT_CHECKBOX',
                   value: 'new-rab-dev-1,new-rab-dev-2,new-rab-dev-3,new-rab-dev-4,new-rab-staging-1,new-rab-staging-2,new-rab-staging-3,new-rab-staging-4,rab-dev-1,rab-staging-1',
                   visibleItemCount: 14
  }
        ...
        container('podman') {
          script {
            ...
            sh """
              ...
              md5sum config/web-config.js
              cp -a config/web-config.js ${DIST_DIR}/config.js
              cp -a ${DIST_DIR} temp_docker_build_dir/
              md5sum temp_docker_build_dir/${DIST_DIR}/config.js
            """
        ...
        script {
          ...
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            ...
            for (namespaces in params.DEPLOY_TO_ENV.tokenize(',')) {
              ...
              println(namespaces)
              helm_values_file="temp_jenkins_workspace/new-rab/rab-webapp/deployToK8s/${deploy_svc_name}-values.yaml"
              sh """
                pwd
              ...
              switch (namespaces) {
                case ['new-rab-dev-1','new-rab-dev-2','new-rab-dev-3','new-rab-dev-4','new-rab-staging-1','new-rab-staging-3']:
                  // API_ADDRS = "http://new-rab-svc-api-app-in-${CONFIG_ENV_SUFFIX}:8080"
                  // SOCKET_ADDRS = "http://new-rab-svc-api-app-in-${CONFIG_ENV_SUFFIX}:9999/socket.io"
                  API_ADDRS = "http://rab-svc-api-app-in-${CONFIG_ENV_SUFFIX}:8080"
                  SOCKET_ADDRS = "http://rab-svc-api-app-in-${CONFIG_ENV_SUFFIX}:9999/socket.io"
                  break
                case ['new-rab-staging-2','new-rab-staging-4']:
                  API_ADDRS = "http://rab-svc-api-app-in-${CONFIG_ENV_SUFFIX}:8080"
                  SOCKET_ADDRS = "http://rab-svc-api-app-master-in-${CONFIG_ENV_SUFFIX}:9999/socket.io"
                  break
                case ['api4-demo-dev-1', 'api4-demo-dev-2', 'api3-demo-dev-1', 'api3-demo-dev-2']:
                  API_ADDRS = "http://rab-svc-api-app-in-${CONFIG_ENV_SUFFIX}:8080"
                  SOCKET_ADDRS = "http://rab-svc-api-app-in-${CONFIG_ENV_SUFFIX}:9999/socket.io"
                  break
                case 'rab-dev-1':
                  ...
                  break
                case 'rab-staging-1':
                  API_ADDRS = 'https://api4.betack.com:443'
                  SOCKET_ADDRS = 'https://api4.betack.com:443'
                  break
              }
...
def doDeployToK8s(deployToEnv, deploySVCName, imageTag, project, chartVersion) {
  ...
  sh """
    ...
    sed -i 's#API_ADDRS#${API_ADDRS}#' ${deploySVCName}-values.yaml
    sed -i 's#SOCKET_ADDRS#${SOCKET_ADDRS}#' ${deploySVCName}-values.yaml
  """
  ...
  echo "部署完成..."
  // 输出访问链接地址
  svcNodePort = svcNodePort = getSVCNodePort(deployToEnv, deploySVCName, 80, CONFIG_ENV_SUFFIX)
