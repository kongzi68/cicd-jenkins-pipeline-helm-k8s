...
pipeline {
  ...
  parameters {
    ...
    extendedChoice description: '发版到选中的运行环境，其中：\nofficial-website-prod-1 是官网的API; \nproduct-rack-prod-1 是策略产品货架API',
                   ...
                   saveJSONParameterToFile: false,
                   type: 'PT_CHECKBOX',
                   value: 'rab-api5-prod-1,rab-api5-uat-1,official-website-prod-1,product-rack-prod-1',
                   visibleItemCount: 10
              ...
              configFileProvider([configFile(fileId: K8S_AUTH, targetLocation: "kube-config.yaml")]){
                switch(deploy_svc_name) {
                  case 'rab-svc-offline-app':
                    svcYaml['service']['ports'] = [5005, 80, 8563, 9090, 9099, 10001, 50053, 50060]
                  break
                  case 'rab-svc-api-app':
                    svcYaml['service']['ports'] = [5005, 80, 8563, 9099, 9999, 12233, 25251, 50053, 50060]
                  break
                }
                switch(deploy_svc_name) {
                  ...
                  case 'rab-svc-offline-app':
                    ...
                    t_javacommand = reserveJavaCommand(k8sRSType: k8s_rs_type, namespaces: namespaces, deploySVCName: deploy_svc_name)
                    // 处理 offline 定时任务作为启动项参数问题
                    // nt_javacommand = t_javacommand.concat(" -Dall.dag.trigger.time='0 0 7 * * ?'")
                    // svcYaml['commandOps'] = [nt_javacommand]
                    svcYaml['commandOps'] = [t_javacommand]
                    println(t_javacommand)
                  break
                  default:
                    chart_version = '0.1.8'
                    k8s_rs_type = 'deploy'
                ...
                if (isFirst) {
                  ...
                  // svcYaml['imagePullSecrets'] = [[name:'bf-harbor-sh-idc-inner'],[name:'bf-harbor-sh-idc-outer']]
                  /* 处理卷创建与挂载问题 */
                  switch(deploy_svc_name) {
                    case ['rab-svc-api-app']:
                      // api创建缓存卷
                    ...
                    case ['rab-svc-offline-app']:
                      ...
                      svcYaml['storage']['capacity'] = ''
                      svcYaml['storage']['dataPVCMountPath'] = ''
                      svcYaml['storage']['dataStorageClassName'] = ''
                      // 日志持久卷
                      svcYaml['storage']['logStorageClassName'] = 'longhorn-fast'
                      // mosek卷
                      svcYaml['storage']['mosekStorageClassName'] = 'longhorn-fast'
                      ...
                      svcYaml['storage']['aloneCapacity'] = '300Gi'
                      svcYaml['storage']['aloneDataPVCMountPath'] = '/opt/rab_backend/data'
                      svcYaml['storage']['aloneDataStorageClassName'] = 'longhorn-fast'
                    break
                    default:
                      // 其它未单独列出的，挂载api创建的卷
                      // 标准卷
