...
pipeline {
  ...
  parameters {
    ...
    extendedChoice description: '发版到选中的运行环境',
                   ...
                   type: 'PT_CHECKBOX',
                   value: 'new-rab-dev-1,new-rab-dev-2,new-rab-dev-3,new-rab-dev-4,new-rab-dev-5,new-rab-staging-1,new-rab-staging-2,new-rab-staging-3,new-rab-staging-4,api4-demo-dev-1,api4-demo-dev-2',
                   visibleItemCount: 15
  }
    ...
    stage('代码编译打包') {
      steps {
        container('frontend') {
          echo '正在构建...'
          script {
            frontendCodeCompile(buildScripts:'npm install --legacy-peer-deps; npm run i18n; npm run build')
          }
        }
      }
    }
      ...
      steps {
        script {
          ...
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            helm_values_file="temp_jenkins_workspace/new-${project}/counsel-nginx/deployToK8s/${deploy_svc_name}-values.yaml"
            // 循环处理需要部署的命名空间
            for (namespaces in params.DEPLOY_TO_ENV.tokenize(',')) {
              ...
              switch (namespaces) {
                case ['new-rab-dev-1','new-rab-dev-2','new-rab-dev-3','new-rab-dev-4','new-rab-staging-1','new-rab-staging-3']:
                  // API_ADDRS = "http://new-rab-svc-api-app-in-${configEnvSuffix}:8080"
                  // SOCKET_ADDRS = "http://new-rab-svc-api-app-in-${configEnvSuffix}:9999/socket.io"
                ...
                case ['new-rab-staging-2','new-rab-staging-4']:
                  API_ADDRS = "http://rab-svc-api-app-in-${configEnvSuffix}:8080"
                  SOCKET_ADDRS = "http://rab-svc-api-app-master-in-${configEnvSuffix}:9999/socket.io"
                break
