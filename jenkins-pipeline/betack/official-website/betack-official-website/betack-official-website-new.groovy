...
def git_address = 'ssh://git@code.betack.com:4022/betack/official-website.git'
...
pipeline {
  ...
  parameters {
    ...
    booleanParam defaultValue: false,
                 ...
                 name: 'IS_ENV_PROD'
    choice (choices: ['betack-official-website', 'betack-index-product'],
    ...
    extendedChoice description: '发版到选中的运行环境',
                   ...
                   type: 'PT_CHECKBOX',
                   value: 'official-website-dev-1,official-website-staging-1,product-rack-dev-1,product-rack-staging-1',
                   visibleItemCount: 8
  }
      ...
      steps {
        ...
        script {
          ...
          switch(params.DEPLOY_SVC_NAME) {
            case 'betack-official-website':
              frontendCodeCompile(buildScripts: "npm run ${BUILD_CMD}; npm run autoclean")
            break
            case 'betack-index-product':
              frontendCodeCompile(buildScripts: "yarn install; npm run ${BUILD_CMD}")
            break
          }
        }
      }
      ...
      steps {
        script {
          ...
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            helm_values_file="temp_jenkins_workspace/${project}/official-website/betack-official-website/deployToK8s/betack-official-website-values.yaml"
            // 循环处理需要部署的命名空间
