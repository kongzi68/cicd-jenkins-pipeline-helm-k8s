...
def git_address = 'ssh://git@code.betack.com:4022/betack/license-ui.git'
  ...
  stages {
    ...
    stage('环境准备') {
      steps {
        ...
        script {
          ENV_TYPE = 'npm'  // 可选值 npm、yarn、pnpm
          selectFrontendBuildEnv(envType:ENV_TYPE, nodeVersion:'18.15.0')
        }
      }
    }
      ...
      steps {
        script {
          ...
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            helm_values_file="temp_jenkins_workspace/${project}/rab-license-ui/deployToK8s/${deploy_svc_name}-values.yaml"
            // 循环处理需要部署的命名空间
            for (namespaces in params.DEPLOY_TO_ENV.tokenize(',')) {
              ...
              API_ADDRS = "http://rab-svc-licence-app-in-${configEnvSuffix}:8080"
              SOCKET_ADDRS = "http://rab-svc-licence-app-in-${configEnvSuffix}:9999/socket.io"
              sh """
