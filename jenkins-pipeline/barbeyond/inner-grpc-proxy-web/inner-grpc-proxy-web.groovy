...
def git_address = 'ssh://git@code.betack.com:4022/beyond/ws-grpc-proxy.git'
// 认证
...
// 部署参考文档
// https://code.betack.com/chenyouguo/rab-engine-x/-/blob/feature/2022-08-30-dag-storage/rust/document/rabbeyond-visual.md
            ...
            try {
              withCredentials([usernameColonPassword(credentialsId: '3d040389-9dfe-4c0d-9dab-9f6487f10409', variable: 'USERPASS')]) {
                sh """
                  ...
                  cd grpc-web-proxy && CARGO_HOME=${WORKSPACE}/.cargo cargo build --release
                """
              }
              echo "构建的jar包 ${deploy_svc_name} 信息如下："
              sh "ls -lh target/release/grpc-web-proxy; pwd"
            } catch(Exception err) {
              echo err.getMessage()
              echo err.toString()
              unstable '构建失败'
          ...
          script{
            for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
              ...
              sh """
                ...
                cp target/release/grpc-web-proxy temp_docker_build_dir/grpc-web-proxy
              """
              ...
              dir("${env.WORKSPACE}/temp_docker_build_dir") {
                docker_file = """
                  ...
                  RUN ln -sf /usr/share/zoneinfo/Asia/Shanghai /etc/localtime && \
                      ...
                      apt-get update && apt-get -y install netcat-openbsd curl wget
                  COPY grpc-web-proxy /opt/betack/grpc-web-proxy
                  WORKDIR /opt/betack
                """.stripIndent()
              ...
              configFileProvider([configFile(fileId: "fd4efaf3-23f9-4f31-a085-3e3baa9618d4", targetLocation: "kube-config.yaml")]){
                ...
                switch(namespaces) {
                  case ['saasdata-dev-1','saasdata-dev-2','saasdata-staging-1','saasdata-staging-2']:
                    ...
                    // saasdata服务集成了sdk-grpc 服务 50053 端口
                ...
                // 有些项目，不需要data-insight-server服务
                // NO_INSIGHT_SERVER=['saasdata-staging-1']  // 我靠，他们又需要用 data-insight-server 
                NO_INSIGHT_SERVER=['']
                if ( ! NO_INSIGHT_SERVER.contains(namespaces)) {
                  // 获取data-insight-server svc 的 NodePort 暴露出来的端口
                  sh """sed -i 's/IS_NEED_INSIGHT_SERVER//g' ${deploy_svc_name}-values.yaml"""
                  try {
                    env.DATA_INSIGHT_NODEPORT = sh (script: """
                      kubectl --kubeconfig=kube-config.yaml -n ${namespaces} get svc data-insight-server-out-${tempEvn} -o jsonpath='{.spec.ports[?(@.port==50056)].nodePort}'
                      """, returnStdout: true).trim()
                    println("data-insight-server NodePort：" + env.DATA_INSIGHT_NODEPORT)
                  } catch(Exception err) {
                    env.DATA_INSIGHT_NODEPORT = ''
                    error "未获取 data-insight-server service 的 nodePort 端口，终止本次部署。"
                  }
                } else {
                  sh """sed -i 's/IS_NEED_INSIGHT_SERVER/#/g' ${deploy_svc_name}-values.yaml"""
                }
...
def doDeployToK8s(deployToEnv, deploySVCName, imageTag, project, chartVersion, namespacePrefix) {
  ...
  sh """
    ...
    sed -i 's#SDK_GRPC_SVC_PORT#${env.GRPC_NODEPORT}#g' ${deploySVCName}-values.yaml
    sed -i 's#DATA_INSIGHT_SVC_PORT#${env.DATA_INSIGHT_NODEPORT}#g' ${deploySVCName}-values.yaml
