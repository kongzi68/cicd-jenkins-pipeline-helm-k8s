...
def git_address = 'ssh://git@code.betack.com:4022/betack/rabbeyond-fe.git'
// 认证
...
pipeline {
  ...
  parameters {
    ...
    extendedChoice description: '请选择本次build类型，默认build:stg',
                   multiSelectDelimiter: ',',
                   name: 'BUILD_TYPE',
                   quoteValue: false,
                   saveJSONParameterToFile: false,
                   type: 'PT_SINGLE_SELECT',
                   value: 'build-dev,build-stg,build-prod',
                   defaultValue: 'build-stg',
                   visibleItemCount: 5
    choice (choices: ['rabbeyond-fe'],
      ...
      steps {
        container('frontend') {
          ...
          sh '''
            ...
            yarn config set sass-binary-site http://npmmirror.com/mirrors/node-sass
            npm install -g n
            # 安装指定版本的node.js 
            export N_NODE_MIRROR=https://mirrors.aliyun.com/nodejs-release/
            n 18.18.2
            hash -r
            npm install -g pnpm@8.6.9
      ...
      steps {
        container('podman') {
          script {
            ...
            case 'rabbeyond-fe':
              DIST_DIR = 'packages/rabbeyond-ui/build'
              break
            default:
              DIST_DIR = 'build'
              break
            ...
            dir("${env.WORKSPACE}/temp_docker_build_dir") {
              docker_file = """
                FROM ${office_registry}/libs/nginx:1.27.1
                LABEL maintainer="colin" version="2.0" datetime="2024-09-19"
              ...
              withCredentials([usernamePassword(credentialsId: "${harbor_auth}", passwordVariable: 'password', usernameVariable: 'username')]) {
                ...
                if (_EXTRANET_HARBOR) {
                  ...
                  sh """
                    ...
                    podman image rm -f ${extranet_image_name}
                  """
                }
              }
            }
            ...
            sh """
              rm ${env.WORKSPACE}/temp_docker_build_dir* -rf
              podman image rm -f ${image_name}
              podman image rm -f ${office_registry}/${project}/${params.DEPLOY_SVC_NAME}:latest
            """
          }
        }
      }
        ...
        script {
          ...
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            ...
            for (namespaces in params.DEPLOY_TO_ENV.tokenize(',')) {
              ...
              println(namespaces)
              helm_values_file="temp_jenkins_workspace/${project}/rabbeyond-fe/deployToK8s/${deploy_svc_name}-values.yaml"
              sh """
                pwd
              ...
              configFileProvider([configFile(fileId: "fd4efaf3-23f9-4f31-a085-3e3baa9618d4", targetLocation: "kube-config.yaml")]){
                ...
                case ['saasdata-dev-1', 'saasdata-dev-2', 'saasdata-staging-1', 'saasdata-staging-2']:
                  API_ADDRS = "http://saas-data-server-out-${CONFIG_ENV_SUFFIX}:8080"
                  SOCKET_ADDRS = "http://inner-grpc-proxy-web-out-${CONFIG_ENV_SUFFIX}:8090"
                  GRPC_ADDRS = "grpc://saas-data-server-out-${CONFIG_ENV_SUFFIX}:50053"
                  break
                default:
                  API_ADDRS = ''
                  SOCKET_ADDRS = "http://inner-grpc-proxy-web-out-${CONFIG_ENV_SUFFIX}:8090"
...
def doDeployToK8s(deployToEnv, deploySVCName, imageTag, project, chartVersion) {
  ...
  sh """
    ...
    sed -i 's#GRPC_ADDRS#${GRPC_ADDRS}#' ${deploySVCName}-values.yaml
  """
...
def getSVCNodePort(namespaces, deploySVCName, podPort, deployENVSuffix, svcNameSalt='out') {
  ...
  */
  try {
    ...
    svc_nodeport = ''
    error "未获取 ${deploySVCName} service 的 nodePort 端口，终止本次部署。"
  }
}
