...
def git_address = 'ssh://git@code.betack.com:4022/betack/betack-ai-management.git'
// 认证
...
pipeline {
  ...
  parameters {
    ...
    choice (choices: ['betack-ai-management'],
        ...
        script {
          ...
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            ...
            for (namespaces in params.DEPLOY_TO_ENV.tokenize(',')) {
              helm_values_file="temp_jenkins_workspace/rabbeyond/${deploy_svc_name}/deployToK8s/${deploy_svc_name}-values.yaml"
