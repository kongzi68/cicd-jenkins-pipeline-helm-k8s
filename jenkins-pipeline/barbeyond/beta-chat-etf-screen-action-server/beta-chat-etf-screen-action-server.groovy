...
pipeline {
  ...
  parameters {
    ...
    extendedChoice description: '请选择本次发版需要部署的服务',
                   ...
                   type: 'PT_CHECKBOX',
                   value: 'beta-chat-etf-screen-action-server',
                   visibleItemCount: 8
        ...
        container('podman') {
          script{
            for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
              echo "创建Dockerfile"
              // 第一次不能复用 beta-chat-etf-screen-action-server:latest
              // FROM ${office_registry}/libs/python:bf-v3.10.14-bookworm
              // ${office_registry}/rabbeyond/beta-chat-baseimage:v-python3.10.14
              // FROM ${office_registry}/rabbeyond/beta-chat-etf-screen-action-server:latest
              dockerFile = """
                FROM ${office_registry}/rabbeyond/beta-chat-etf-screen-action-server:latest
                LABEL maintainer="colin" version="1.0" datetime="2024-07-19"
                ADD *.whl /opt/betack/
                WORKDIR /opt/betack/
        ...
        script {
          ...
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            ...
            for (namespaces in params.DEPLOY_TO_ENV.tokenize(',')) {
              helm_values_file="temp_jenkins_workspace/${project}/beta-chat-etf-screen-action-server/deployToK8s/${deploy_svc_name}-values.yaml"
