...
pipeline {
  ...
  parameters {
    ...
    extendedChoice description: '请选择本次发版需要部署的服务',
                   ...
                   type: 'PT_CHECKBOX',
                   value: 'beta-chat-action-server1',
                   visibleItemCount: 8
          ...
          script{
            for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
              ...
              if (params.IS_ENABLED_UPDATE_DEPENDENCIES) {
                ...
                dockerFile = """
                  FROM ${office_registry}/rabbeyond/beta-chat-action-server1:latest
                  LABEL maintainer="colin" version="1.0" datetime="2024-07-17"
                  COPY src /opt/betack/app
                  WORKDIR /opt/betack/app
                  CMD ["python", "endpoint/__main__.py"]
        ...
        script {
          ...
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            ...
            for (namespaces in params.DEPLOY_TO_ENV.tokenize(',')) {
              helm_values_file="temp_jenkins_workspace/${project}/beta-chat-action-server/deployToK8s/${deploy_svc_name}-values.yaml"
              ...
              svcYaml['storage']['dataPVCNameInfix'] = 'action1'
              svcYaml['storage']['dataPVCMountPath'] = '/opt/betachat/cache'
