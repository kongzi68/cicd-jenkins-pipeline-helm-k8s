...
// 项目
def project = "etf"  // HARrab镜像仓库中的项目名称
def git_address = "ssh://git@code.betack.com:4022/betack/etf-explorer.git"
...
pipeline {
  ...
  parameters {
    ...
    extendedChoice description: '请选择本次发版需要部署的服务',
                   ...
                   saveJSONParameterToFile: false,
                   type: 'PT_CHECKBOX',
                   value: 'etf-explorer-app-svc,etf-explorer-data-sync,etf-explorer-migration',
                   visibleItemCount: 6
    string description: 'etf-explorer-migration的参数，多个参数用空格分隔，【注意】每次都会默认清空上一次变量参数，如果要保持上一次变量参数，请重新填写',
           name: 'etf_EXPLORER_MIGRATION',
           trim: true
    extendedChoice description: '发版到选中的运行环境',
                   ...
                   saveJSONParameterToFile: false,
                   type: 'PT_CHECKBOX',
                   value: 'etf-dev-1,etf-dev-2,etf-staging-1',
                   visibleItemCount: 10
    booleanParam defaultValue: false,
    ...
    stage('部署服务') {
      ...
      steps {
        script {
          ...
          // 循环处理需要部署的服务
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            helm_values_file="temp_jenkins_workspace/etf/etf-explorer/deployToK8s/${deploy_svc_name}-values.yaml"
            // 循环处理需要部署的命名空间
            for (namespaces in params.DEPLOY_TO_ENV.tokenize(',')) {
              ...
              switch(deploy_svc_name) {
                case 'etf-explorer-migration':
                  chart_version = '0.1.4'
                  k8s_rs_type = 'job'
                  // job任务特殊处理
                  svcYaml['svcJARPKGOptions'] = params.etf_EXPLORER_MIGRATION
                break
                default:
