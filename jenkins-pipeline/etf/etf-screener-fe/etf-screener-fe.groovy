...
def git_address = 'ssh://git@code.betack.com:4022/betack/etf-screener-fe.git'
...
pipeline {
  ...
  parameters {
    ...
    choice (choices: ['etf-screener-fe'],
    ...
    stage('环境准备') {
      steps {
        container('frontend') {
          ...
          script {
            ENV_TYPE = 'npm'  // 可选值 npm、pnpm、yarn
    ...
    stage('代码编译打包') {
      steps {
        container('frontend') {
          echo '正在构建...'
          script {
            frontendCodeCompile(buildScripts: "npm install --force; npm run build;")
          }
        }
      }
    }
      ...
      steps {
        script {
          ...
          for (deploy_svc_name in params.DEPLOY_SVC_NAME.tokenize(',')) {
            helm_values_file="temp_jenkins_workspace/${project}/etf-screener-fe/deployToK8s/${deploy_svc_name}-values.yaml"
            // 循环处理需要部署的命名空间
            for (namespaces in params.DEPLOY_TO_ENV.tokenize(',')) {
              ...
              API_ADDRS = "http://etf-explorer-app-svc-out-${configEnvSuffix}:8080"
              SOCKET_ADDRS = "http://etf-explorer-app-svc-out-${configEnvSuffix}:9999/socket.io"
              sh """
